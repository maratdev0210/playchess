...
export default function ChessBoardReversed({
  ...
  let reversedCoords = new Array(8).fill(null);
  for (let i = 0; i < 8; i++) {
    reversedCoords[i] = coordinates[7 - i];
  }
                ...
                {row.map((tile, colIndex) => {
                  return (
                    ...
                    >
                      {tile !== null && (
                        <img
                          className={`${selectedSquarePosition !== undefined && selectedSquarePosition == SQUARES[63 - rowIndex * 8 - colIndex] ? "bg-yellow-200" : ""} ${isInCheck && tile.type == "k" && tile.color == turn ? "bg-red-500" : ""}`}
